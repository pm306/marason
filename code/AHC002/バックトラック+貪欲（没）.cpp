...
void routing(int ci, int cj, int score, set<int> used, string order){
    ...
    score += point[ci][cj];
    if(chmax(max_score, score)) result = order;

    //ルーティング（貪欲法）。得点の高い方向から探索する
    //スコアの高い順に進める
    vector<pii> dir_list; //score, direction_index
    rep(dir, 4){
        auto [ni, nj] = move(ci, cj, dir);
        if(ni == -1)continue;
        if(used.find(tile_idx[ni][nj]) != used.end())continue; //使用済み
        dir_list.emplace_back(point[ni][nj], dir);
    }
    sort(all(dir_list), greater<pii>());

    for(auto [add_score, dir] : dir_list){
        auto [ni, nj] = move(ci, cj, dir);
        order += idx_to_char[dir];
        routing(ni, nj, score, used, order);
        order.pop_back();
    }
