...
vector<int> last; //各コンテストを最後に開催した日
...
//出力
void output(){
    for(auto& t : T)cout << t + 1 << ln << flush;
}
...
int calcLocalSearch(int pre_score, int update_day, int after_contest){
...
//コンテスト情報を更新する
void update_info(int before_contest, int after_contest, int day){
    //集合からの削除と追加
    contest_days[before_contest].erase(day);
    contest_days[after_contest].insert(day);
    T[day] = after_contest;
}
...
int main(){
    ...
    //初期解をランダムに生成し、暫定スコアを求める
    int score = 0;
    for(int d=0; d<D; d++){
        T[d] = random(0, CONTEST_NUM);
        score = calc_score(score, d, T[d], last);
    }
    ...
    while(timeCheck()){
        int day = random(0, D);
        int after_contest = random(0, CONTEST_NUM);
        int tmp_score = calcLocalSearch(score, day, after_contest);
        //改善される場合は更新
        if(chmax(score, tmp_score)){
            update_info(T[day], after_contest, day);
        }
    }
