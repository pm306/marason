...
double calcScore(int idx){
    ...
    return (1.0 - pow((1.0 - (double)min(area, r[idx]) / max(area, r[idx])), 2)) / n * 1e9;
}
...
bool isInnerErea(int idx, int dir){
    ...
    if(dir == 0) return a[idx] > 0;
    if(dir == 1)return d[idx] < EDGE_SIZE;
    if(dir == 2)return c[idx] < EDGE_SIZE;
    if(dir == 3)return b[idx] > 0;
}
...
void solve(){
    ...
    while(timeCheck(490)){
        ...
        int idx = random(0, n); //加工する長方形
        int dir = random(0, 4); //加工する方向
        int order = random(0, 2); //加工内容。0:=拡大、1:=平行移動、2:=縮小
